...
const coffeeBeansInput = (event) => {
  ...
  if (
    last2Inputs.includes(beansElement) &&
    last2Inputs.includes(ratioElement)
  ) {
    waterValue = beansValue * ratioValue;
    waterElement.value = waterValue.toFixed(2);
  } else if (
    last2Inputs.includes(beansElement) &&
    last2Inputs.includes(waterElement)
  ) {
...
const isSunRise = async () => {
  try {
    ...
    const response = await fetch(`https://api.sunrise-sunset.org/json?lat=${position.coords.latitude}&lng=${position.coords.longitude}&formatted=0`);
    if (!response.ok) {
    ...
    if (now < sunrise) {
      ...
      infoBoxElement.innerHTML =
        " ☕ Just now it's an ideal time to have a coffee.";
    } else if (hours < 16) {
      infoBoxElement.innerHTML = " ☕ You're fine having a coffee now.";
    } else if (now < sunset) {
      infoBoxElement.innerHTML =
        " ⚠️ Beware, you shouldn't drink coffee at least 6 hours before sleep.";
    } else {
      infoBoxElement.innerHTML =
        "❌ It's already night, you definitely shouldn't be drinking coffee now.";
    }
  } catch (error) {
